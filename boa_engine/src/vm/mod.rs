    ...
    fn execute_instruction(&mut self) -> JsResult<ShouldExit> {
        ...
        match opcode {
            ...
            Opcode::SuperCall => {
                ...
                let result =
                    super_constructor.__construct__(&arguments, &new_target.into(), self)?;
                let result_object = result.as_object().expect("constructor must return object");

                initialize_instance_elements(result_object, &active_function, self)?;
